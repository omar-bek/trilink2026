...
@section('title', __('company.users.edit'))

@section('content')

<x-dashboard.page-header :title="__('company.users.edit')" :subtitle="$user->email" :back="route('company.users')" />
...
<form method="POST" action="{{ route('company.users.update', $user->id) }}" id="user-form" class="space-y-6">
    @csrf @method('PATCH')
