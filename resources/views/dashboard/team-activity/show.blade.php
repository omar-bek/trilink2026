...
<x-dashboard.page-header :title="__('team_activity.entry') . ' #' . $log->id" :back="route('dashboard.team-activity.index')" />
...
<div class="space-y-6 max-w-5xl">
    ...
    <div class="bg-surface border border-th-border rounded-[16px] p-[25px]">
        <div class="flex items-center gap-3 mb-5">
            ...
            <div>
                <h3 class="text-[15px] font-bold text-primary leading-tight">{{ __('team_activity.entry') }} #{{ $log->id }}</h3>
        ...
        <dl class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5 text-[13px]">
            <div>
                <dt class="text-[10px] font-bold text-faint uppercase tracking-wider">{{ __('team_activity.when') }}</dt>
                <dd class="text-primary mt-1.5 font-mono text-[12px]">{{ $log->created_at?->format('Y-m-d H:i:s') }}</dd>
            </div>
            <div>
                <dt class="text-[10px] font-bold text-faint uppercase tracking-wider">{{ __('team_activity.member') }}</dt>
                <dd class="text-primary mt-1.5">
                    {{ trim(($log->user?->first_name ?? '') . ' ' . ($log->user?->last_name ?? '')) ?: '—' }}
                    <span class="text-muted text-[12px]">({{ $log->user?->email ?? '—' }})</span>
                    @if($roleLabel)
                        <span class="ms-1 inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold text-muted bg-surface-2 border border-th-border">{{ $roleLabel }}</span>
                    @endif
                </dd>
            </div>
            <div>
                <dt class="text-[10px] font-bold text-faint uppercase tracking-wider">{{ __('team_activity.resource') }}</dt>
                <dd class="text-primary mt-1.5 font-mono text-[12px]">{{ $log->resource_type }}<span class="text-faint">#</span>{{ $log->resource_id }}</dd>
            </div>
            <div>
                <dt class="text-[10px] font-bold text-faint uppercase tracking-wider">{{ __('team_activity.ip') }}</dt>
                <dd class="text-primary mt-1.5 font-mono text-[12px]">{{ $log->ip_address }}</dd>
            </div>
            <div class="md:col-span-2">
                <dt class="text-[10px] font-bold text-faint uppercase tracking-wider">User Agent</dt>
                <dd class="text-body mt-1.5 text-[11px] font-mono break-all bg-surface-2 border border-th-border rounded-[10px] px-3 py-2">{{ $log->user_agent }}</dd>
            </div>
        </dl>
    </div>
    ...
    <div class="bg-surface border border-th-border rounded-[16px] p-[25px]">
        <div class="flex items-center gap-3 mb-5">
            ...
            <h3 class="text-[15px] font-bold text-primary">{{ __('team_activity.changes') }}</h3>
        </div>

        @if(empty($keys))
        <div class="rounded-[12px] border border-dashed border-th-border bg-surface-2/40 px-6 py-8 text-center">
            <p class="text-[13px] text-muted">{{ __('team_activity.no_changes') }}</p>
        </div>
        @else
            ...
            <table class="w-full text-[12px]">
                <thead class="bg-surface">
                    <tr class="text-faint text-[10px] uppercase tracking-[0.08em]">
                        <th class="text-start px-4 py-3 w-48 font-bold">{{ __('team_activity.field') }}</th>
                        <th class="text-start px-4 py-3 font-bold">{{ __('team_activity.before') }}</th>
                        <th class="text-start px-4 py-3 font-bold">{{ __('team_activity.after') }}</th>
                    </tr>
                </thead>
