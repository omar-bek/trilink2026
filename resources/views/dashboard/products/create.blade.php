...
@section('title', __('catalog.add_product'))

@section('content')

<x-dashboard.page-header :title="__('catalog.add_product')" :subtitle="__('catalog.my_products_subtitle')" />
...
<form method="POST" action="{{ route('dashboard.products.store') }}" enctype="multipart/form-data" class="space-y-5">
    @csrf
    @include('dashboard.products._form', ['product' => null, 'categories' => $categories])
