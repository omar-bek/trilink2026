...
@section('title', __('admin.tax_rates.new'))

@section('content')

<x-dashboard.page-header :title="__('admin.tax_rates.new')" :subtitle="__('admin.tax_rates.subtitle')" :back="route('admin.tax-rates.index')" />

<x-admin.navbar active="tax-rates" />

<form method="POST" action="{{ route('admin.tax-rates.store') }}" class="space-y-5">
    @csrf
    @include('dashboard.admin.tax-rates._form', ['taxRate' => null, 'categories' => $categories])
