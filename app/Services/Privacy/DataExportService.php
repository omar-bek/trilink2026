...
class DataExportService
{
    ...
    private function addUploadedFiles(ZipArchive $zip, User $user): array
    {
        ...
        if ($user->company) {
            $companyDocs = CompanyDocument::where('company_id', $user->company_id)->get();
            ...
            $icvs = IcvCertificate::where('company_id', $user->company_id)->get();
        ...
        return $this->consents->ledgerFor($user)
            ->map(function (Consent $c) {
                ...
                if ($c->privacy_policy_version_id) {
                    $version = PrivacyPolicyVersion::find($c->privacy_policy_version_id);
                    if ($version) {
                        $row['policy_snapshot'] = [
